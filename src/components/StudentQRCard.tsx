...
import { X, Printer, FileImage, FileText, Loader2 } from 'lucide-react';
...
export default function StudentQRCard({ student, onClose }: StudentQRCardProps) {
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          @media print { body { background: white; } #student-qr-card { box-shadow: none !important; border: 1px solid #e5e7eb !important; } }
        </style>
      </head>
      <body>
        ${cardHtml}
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`SMARTBUS-${student.studentId}-QR.pdf`);
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="dark:bg-navy-800 bg-white dark:border-white/10 border-surface-200 border rounded-2xl w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 flex flex-col items-center">
          <div ref={cardRef} id="student-qr-card" className="bg-white rounded-2xl p-6 text-center shadow-lg w-[280px]">
            <div className="mb-3">
            ...
            <div className="my-3 flex justify-center">
              <div className="bg-white p-3 rounded-xl border-2 border-gray-200">
                <QRCodeCanvas value={student.qrCode} size={160} level="H" includeMargin={false} />
              </div>
            </div>

            <p className="text-[10px] text-gray-400 font-mono mb-2">{student.qrCode}</p>

            <p className="text-sm font-bold text-gray-900">{student.fullName}</p>
            <p className="text-[10px] text-gray-500">{student.studentId} | Class: {student.class}{student.section ? '-' + student.section : ''}</p>
