  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            <div className="flex items-center gap-4 flex-1">
              ...
              <div>
                ...
                <p className="text-sm dark:text-gray-300 text-surface-600 mt-1">Vehicle: {activeSOS.vehicleId} | Driver: {activeSOS.driverId}</p>
                <p className="text-xs dark:text-gray-400 text-surface-500 mt-1">{activeSOS.reason}</p>
                <p className="text-xs dark:text-gray-400 text-surface-500">Location: {activeSOS.location.lat.toFixed(4)}°N, {activeSOS.location.lng.toFixed(4)}°E</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-center px-4 py-2 dark:bg-navy-700/50 bg-surface-100 rounded-xl">
                <p className="text-[10px] dark:text-gray-400 text-surface-500">Escalation Level</p>
                <p className="text-lg font-bold text-amber-400">{activeSOS.escalationLevel.toUpperCase()}</p>
              </div>
              <div className="text-center px-4 py-2 dark:bg-navy-700/50 bg-surface-100 rounded-xl">
                <p className="text-[10px] dark:text-gray-400 text-surface-500">Time Remaining</p>
          ...
          {studentsOnBus.length > 0 && (
            <div className="mt-4 pt-4 border-t border-red-500/20">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {studentsOnBus.map(s => (
                  <div key={s.id} className="flex items-center gap-2 p-2 dark:bg-navy-700/30 bg-surface-50 rounded-lg">
                    <div className="w-6 h-6 rounded-full bg-red-500/20 flex items-center justify-center">
                      <span className="text-[8px] font-bold text-red-400">{s.fullName[0]}</span>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold dark:text-white text-surface-900">{s.fullName}</p>
                      <p className="text-[8px] dark:text-gray-400 text-surface-500">{s.studentId}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="glass-card p-6">
        <h3 className="text-sm font-bold dark:text-white text-surface-900 mb-6 flex items-center gap-2">
        ...
        {sosAlerts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-bold dark:text-white text-surface-900 mb-2">No Active Emergencies</h4>
            <p className="text-sm dark:text-gray-400 text-surface-500">All systems operating normally</p>
          </div>
        ) : (
            ...
            {sosAlerts.map((sos) => {
              ...
              return (
                <div key={sos.id} className={`rounded-2xl border p-6 ${
                  ...
                  'dark:bg-navy-700/30 bg-surface-50 dark:border-white/5 border-surface-200'
                }`}>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className={`status-badge border ${getStatusBadge(sos.status)}`}>{sos.status.toUpperCase()}</span>
                      <span className="text-xs dark:text-gray-400 text-surface-500">{sos.id} — {sos.time}</span>
                    </div>
                    <span className="text-xs dark:text-gray-400 text-surface-500">{sos.vehicleId} — {driver?.fullName || sos.driverId}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className={`rounded-xl p-4 border ${
                      sos.escalationLevel === 'primary' && !sos.primaryResponded
                        ? 'bg-amber-500/10 border-amber-500/30' : 'dark:bg-navy-700/30 bg-surface-50 dark:border-white/5 border-surface-200'
                    }`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold dark:text-white text-surface-900">Level 1 — School Admin</span>
                        {sos.primaryResponded ? (
                          ...
                          <span className="text-[10px] dark:text-gray-400 text-surface-500">Skipped</span>
                        )}
                      </div>
                      <p className="text-xs dark:text-gray-300 text-surface-600">{sos.primaryContact.name}</p>
                      <p className="text-[10px] dark:text-gray-400 text-surface-500">{sos.primaryContact.phone}</p>
                      {sos.escalationLevel === 'primary' && !sos.primaryResponded && (
                    ...
                    <div className={`rounded-xl p-4 border ${
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold dark:text-white text-surface-900">Level 2 — Secondary</span>
                        {sos.secondaryResponded ? (
                          <span className="text-[10px] text-emerald-400 flex items-center gap-1"><CheckCircle className="w-3 h-3" />Responded</span>
                        ) : sos.escalationLevel === 'secondary' ? (
                          <span className="text-[10px] text-amber-400 animate-pulse">Waiting...</span>
                        ) : (
                          <span className="text-[10px] dark:text-gray-400 text-surface-500">Waiting</span>
                        )}
                      </div>
                      <p className="text-xs dark:text-gray-300 text-surface-600">{sos.secondaryContact.name}</p>
                      <p className="text-[10px] dark:text-gray-400 text-surface-500">{sos.secondaryContact.phone}</p>
                      {sos.escalationLevel === 'secondary' && !sos.secondaryResponded && (
                        <div className="grid grid-cols-2 gap-2 mt-3">
                    ...
                    <div className={`rounded-xl p-4 border ${
                      sos.escalationLevel === 'authority' && !sos.authorityResponded
                        ? 'bg-red-500/10 border-red-500/30' : 'dark:bg-navy-700/30 bg-surface-50 dark:border-white/5 border-surface-200'
                    }`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold dark:text-white text-surface-900">Level 3 — Authority</span>
                      ...
                      <p className="text-xs dark:text-gray-300 text-surface-600">{sos.authorityContact.name}</p>
                      <p className="text-[10px] dark:text-gray-400 text-surface-500">{sos.authorityContact.phone}</p>
                      {sos.escalationLevel === 'authority' && (
                        <div className="mt-3">
                  ...
                  <div className="flex items-center gap-3 mt-4 pt-4 border-t dark:border-white/5 border-surface-200">
      ...
      {/* Emergency History */}
      <div className="glass-card p-6">
        <h3 className="text-sm font-bold dark:text-white text-surface-900 mb-4">Emergency Log</h3>
        <div className="space-y-2">
          {sosAlerts.map((sos) => (
            <div key={sos.id} className="flex items-center gap-3 p-3 rounded-xl dark:bg-navy-700/30 bg-surface-50">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold dark:text-white text-surface-900">{sos.vehicleId} — {sos.reason}</p>
                <p className="text-[10px] dark:text-gray-400 text-surface-500">{sos.time} — {sos.status}</p>
              </div>
